...
struct StorageRowGateLogicTests {
    ...
    @Test func groupsAccountAxis_allFourCells() {
        ...
        for row in rows {
            #expect(
                StorageRowGateLogic.isVisible(
                    isConfigured: true, isSecondaryActive: false,
    ...
    /// Los dos guards de arriba mandan sobre el término nuevo, y ninguno es cosmético.
    ///
    /// · **Secundaria**: la fila describe y opera al DUEÑO; una invitada no suelta la cuenta de nadie, y
    ///   además el espejo local de la asociación vive en el `UserDefaults` del dueño.
    /// · **Sin configurar**: `CloudMigrationController.shared` es `nil`, la pantalla degrada al mensaje
    ///   genérico y la sección de grupos NO se monta ⇒ la fila abriría a una puerta sin nada detrás.
    @Test func groupsAssociationDoesNotOverrideTheTwoGuards() {
        for remote in [true, false] {
            for engaged in [true, false] {
                #expect(!StorageRowGateLogic.isVisible(
                    isConfigured: true, isSecondaryActive: true,
                    remoteEnabled: remote, isEngaged: engaged,
                    hasGroupsAccountToDetach: true), """
                    la asociación de grupos abrió la fila en sesión SECUNDARIA (remote=\(remote) \
                    engaged=\(engaged)): esa fila es del dueño, y la invitada no suelta su cuenta.
                    """)
                #expect(!StorageRowGateLogic.isVisible(
                    isConfigured: false, isSecondaryActive: false,
                    remoteEnabled: remote, isEngaged: engaged,
                    hasGroupsAccountToDetach: true), """
